...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiError> handleValidation(MethodArgumentNotValidException exception, HttpServletRequest request) {
        ...
        return buildResponse(HttpStatus.BAD_REQUEST, "Dados de entrada inválidos", request.getRequestURI(), fieldErrors);
    }
    ...
    private ResponseEntity<ApiError> buildResponse(HttpStatus status, String message, String path, Map<String, String> fieldErrors) {
        ApiError error = new ApiError(status.value(), status.getReasonPhrase(), message, path, fieldErrors);
